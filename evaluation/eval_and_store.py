"""
GenCast 0.25 Degree Evaluation Script - Sparse Evaluation Schedule with Prediction Saving

Efficient version that evaluates only at 6:00 UTC every 15 days starting from January 1st.
Uses local ERA5 data at native 0.25 degree resolution and evaluates on specified year.
...
def save_predictions_and_targets(predictions, targets, init_timestamp, output_dir):
    ...
    try:
        ...
        # Save predictions
        pred_filename = f"predictions_025deg_{timestamp_str}.nc"
        pred_path = output_dir / pred_filename
        
        # Add metadata to predictions
        predictions.attrs.update({
            'description': 'GenCast 0.25 degree ensemble predictions',
            'model_resolution': '0.25 degrees',
        ...
        # Save targets
        target_filename = f"targets_025deg_{timestamp_str}.nc"
        target_path = output_dir / target_filename
        
        # Add metadata to targets
        targets.attrs.update({
            'description': 'ERA5 verification targets at 0.25 degree resolution',
            'model_resolution': '0.25 degrees',
            'initialization_time': str(init_timestamp),
            'forecast_days': ROLLOUT_DAYS,
            'evaluation_schedule': f'Sparse evaluation every {EVAL_INTERVAL_DAYS} days at {EVAL_HOUR:02d}:00 UTC',
...
def main():
    parser = argparse.ArgumentParser(description='GenCast 0.25 Degree Sparse Evaluation with Local ERA5 Data and Prediction Saving')
    ...
    parser.add_argument('--output_dir', type=str, default='predictions_025deg_sparse_output', 
                        help='Directory to save predictions (default: predictions_025deg_sparse_output)')
    ...
    print("Starting GenCast 0.25 Degree Sparse Evaluation with Prediction Saving")
    print("=" * 80)
    print(f"Configuration:")
    print(f"  Model: 0.25 degree resolution (native, no downsampling)")
    ...
    # Load 0.25 degree model checkpoint
    params_file = "GenCast 0p25deg <2019.npz"
    print(f"Loading 0.25 degree model: {params_file}")
    ...
    # Note: Using local ERA5 data at native 0.25 degree resolution
    print("Using local ERA5 data at native 0.25 degree resolution (no downsampling)")
        ...
        try:
            ...
            try:
                ...
                eval_data = get_batch_contiguous(
                    ...
                    batch_size=1, 
                    window=window,
                    downsample=1  # Use native resolution for 0.25 degree (no downsampling)
                )
            ...
            if args.save_predictions:
                ...
                if saved_successfully:
                    ...
                    prediction_files.append({
                        ...
                        'predictions_file': f"predictions_025deg_{timestamp_str}.nc",
                        'targets_file': f"targets_025deg_{timestamp_str}.nc"
                    })
    ...
    if crps_results:
        ...
        print(f"\nSparse Evaluation Summary (0.25 Degree Model):")
        print(f"Evaluation schedule: Every {EVAL_INTERVAL_DAYS} days at {EVAL_HOUR:02d}:00 UTC")
        ...
        results_filename = f'crps_results_025deg_sparse_{EVAL_YEAR}_{EVAL_INTERVAL_DAYS}day_{EVAL_HOUR:02d}utc.csv'
        results_df.to_csv(results_filename, index=False)
        print(f"\nResults saved to {results_filename}")
        
        # Save timing results
        timing_df = pd.DataFrame(timing_results)
        timing_filename = f'timing_results_025deg_sparse_{EVAL_YEAR}_{EVAL_INTERVAL_DAYS}day_{EVAL_HOUR:02d}utc.csv'
        ...
        if prediction_files:
            ...
            pred_files_filename = f'prediction_files_025deg_sparse_{EVAL_YEAR}_{EVAL_INTERVAL_DAYS}day_{EVAL_HOUR:02d}utc.csv'
        ...
        try:
            ...
            histogram_filename = f'crps_histogram_025deg_sparse_{EVAL_YEAR}_{EVAL_INTERVAL_DAYS}day_{EVAL_HOUR:02d}utc.png'
        ...
        try:
            ...
            timeseries_filename = f'crps_timeseries_025deg_sparse_{EVAL_YEAR}_{EVAL_INTERVAL_DAYS}day_{EVAL_HOUR:02d}utc.png'
